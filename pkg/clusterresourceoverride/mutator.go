...
func (m *podMutator) OverrideMemory(resources *corev1.ResourceRequirements) {
	...
	if m.IsMemoryFloorSpecified() && overridden.Cmp(*m.floor.Memory) < 0 {
		klog.V(5).Infof("%s: %s pod limit %q below namespace limit; setting limit to %q", PluginName, corev1.ResourceMemory, overridden.String(), m.floor.Memory.String())
...
func (m *podMutator) OverrideCPULimit(resources *corev1.ResourceRequirements) {
	...
	if m.IsCpuFloorSpecified() && overridden.Cmp(*m.floor.CPU) < 0 {
		klog.V(5).Infof("%s: %s pod limit %q below namespace limit; setting limit to %q", PluginName, corev1.ResourceCPU, overridden.String(), m.floor.CPU.String())
